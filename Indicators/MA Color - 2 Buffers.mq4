...
//External setting
input int MAPeriod = 80; // MA Period 
...
//--- indicator buffers mapping
   ...
   //SetIndexBuffer(0,Sideway); // associated array with buffer   
...
//----since we chekcing i+1 , for the first drop ont he indicator , we need to run 1 less bar to check(to prevent array out of range), so we decrement limit once again by 1 
   ...
   for(int i=limit-1;i>=0;i--) // calculate Green/Red histogram  (for ease of logic thinking) 
   {
      ...
      //double prev2 = iMA(Symbol(),Period(),MAPeriod,MAShift,MAMethod,MAAppliedTo,i+2);
      ...
      {       
         ...
         //if(prev2<previous) // tipping point
         if(Downtrend[i+1] == EMPTY_VALUE)
         {
            Downtrend[i+1]=previous; // Red Histogram(above the zero line)
            //Uptrend[i+1] = EMPTY_VALUE; // since we updating real-time and a candle can start Growing but than Shrinking, we need to update to EMPTY_VALUE         
         }
         //Sideway[i] = current;
      }
      ...
      else if(current>=previous) // MA is increasing 
      {
         ...
         {
            Uptrend[i+1]=previous; // Red Histogram(above the zero line)
            //Downtrend[i+1] = EMPTY_VALUE; // since we updating real-time and a candle can start Growing but than Shrinking, we need to update to EMPTY_VALUE         
         }
